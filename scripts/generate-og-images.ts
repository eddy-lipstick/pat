...
async function generateOGImages() {
  const outputDir = './public/og/case-studies';
  await fs.mkdir(outputDir, { recursive: true });

  const caseStudiesDir = './src/content/case-studies';
  const files = await fs.readdir(caseStudiesDir);

  const fontData = await fs.readFile('./public/fonts/inter-latin-400-normal.woff');
  ...
  for (const file of files) {
    ...
    console.log(`\nProcessing case study: ${id}`);

    try {
      // Handle the background image
      let baseImage;

      if (frontmatter.cover_image?.src) {
        const imagePath = path.join('./public', frontmatter.cover_image.src);
        ...
        try {
          ...
          // Create base image with cover image and overlay
          baseImage = await sharp(imagePath)
            ...
            .composite([
              {
                input: Buffer.from(
                  `<svg><rect width="1200" height="630" fill="rgba(3, 0, 55, 0.5)"/></svg>`,
                ),
                blend: 'multiply',
              },
            ])
        ...
        console.log('No cover image specified, using solid background');
        baseImage = await sharp({
          create: {
            width: 1200,
            height: 630,
            ...
            children: [
              {
                ...
                props: {
                  ...
                  children: [
                    // Client tag - changed to flex instead of inline-block
                    {
                      type: 'div',
                      props: {
                        style: {
                  ...
                  children: [
                    {
                      ...
                      props: {
                        ...
                        children: [
                          ...
                          // ... add all other path elements similarly
                        ],
                      },
                    },
                  ],
      ...
      const textOverlay = resvg.render().asPng();
      ...
      await fs.writeFile(path.join(outputDir, `${id}.png`), finalImage);

      console.log(`Successfully generated OG image for ${id}`);
    } catch (error) {
      console.error(`Error generating OG image for ${id}:`, error);
    }
  }
